from app.dashboard.reader.models import readerDetails
...
# from app.dashboard.reader import views as vw
...
def indexVIEW(request):
    ...
    # print(request.user.Category)
    if (request.user.Category == 'Writer' and not(writerDetails.objects.filter(User_Name=request.user))):
        writer = writerDetails(User_Name=request.user,
                               follower=0, isValidated=False)
        writer.save()
        details = vw.authorDetails(request.user)
        context = {
            'followers': details[0],
            'isValidated': details[1],
        }
    elif (request.user.Category == 'Reader' and not(readerDetails.objects.filter(User_Name=request.user))):
        reader = readerDetails(User_Name=request.user,
                               )
...
def testVIEW(request):
    ...
    for i in posts:
        ...
        details = vw.authorDetails(i.User_Name)
        details_list.append(
            {'followers': details[0], 'isValidated': details[1]})
    required_list = zip(posts, details_list)
    return render(request, 'index.html', {'list': required_list})
